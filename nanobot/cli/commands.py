...
def gateway(
    ...
    agent = AgentLoop(
        ...
        max_exempt_rounds=config.agents.defaults.max_exempt_rounds,
        max_message_calls_per_turn=config.agents.defaults.max_message_calls_per_turn,
    ...
    async def on_heartbeat_execute(tasks: str) -> str:
        """Phase 2: execute heartbeat tasks through full agent loop."""
    ...
    async def on_heartbeat_notify(response: str) -> None:
        """Deliver heartbeat response to an external channel when available."""
        from nanobot.bus.events import OutboundMessage
        channel, chat_id = _pick_heartbeat_target()
        if channel == "cli":
            return
